...
- Each website's `yarn dev` and `yarn build` scripts _should_ still work as expected, as the new CLI assumes that marko templates need to be compiled from `../packages` and `../node_modules/@parameter1` (assuming a standard site working directory of `sites/domain.com`). If this needs to be adjusted, change the site's `dev` script `--compile-dir` option
