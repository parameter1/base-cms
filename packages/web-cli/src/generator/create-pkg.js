...
module.exports = ({ projectName, withBootstrap }) => {
  const markoWebVersion = dependencies['@parameter1/base-cms-marko-web'];
  if (!markoWebVersion) throw new Error('Unable to obtain web package version from dependencies.');

  const deps = {
    [name]: `~${version}`,
    '@parameter1/base-cms-marko-web': `~${markoWebVersion.replace('^', '')}`,
    '@parameter1/base-cms-object-path': `^${objectPathPkg.version}`,
    graphql: '^14.1.1',
    'graphql-tag': '^2.10.1',
  };
  if (withBootstrap) {
